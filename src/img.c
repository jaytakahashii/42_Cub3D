...
{
	window_init(game);
	game->north.img = mlx_xpm_file_to_image(game->mlx, game->map_info->no, &game->north.width, &game->north.height);
	game->south.img = mlx_xpm_file_to_image(game->mlx, game->map_info->so, &game->south.width, &game->south.height);
	...
	if (!game->north.data || !game->south.data || !game->west.data || !game->east.data)
		return (1);
	return (0);
}
